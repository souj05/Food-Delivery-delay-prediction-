...
app = FastAPI(
    ...
    description="Machine Learning API to predict whether a food delivery will be delayed based on customer, order, restaurant, weather, and traffic information.",
    version="1.0.0",
)
...
def build_features(data: OrderInput) -> pd.DataFrame:
    ...
    d["traffic_weather_score"] = (
        d["traffic_level_score"] * d["weather_severity_score"]
    )

    d["avg_item_value"] = d["order_value"] / max(d["number_of_items"], 1)

    d["discount_percentage"] = (
        d["discount_amount"] / (d["order_value"] + 1)
    ) * 100

    d["fee_per_km"] = (
        d["delivery_fee"] / max(d["delivery_distance_km"], 0.1)
    )
...
def home():
    return {
        "project": "Food Delivery Delay Prediction API",
        "author": "Sowjanya Tadimarri",
        "version": "1.0.0",
        "status": "Running"
    }
...
def predict_delay(order: OrderInput):
    ...
    if order.traffic_level_score >= 7:
        risks.append("High traffic")

    if order.weather_severity_score >= 7:
        risks.append("Severe weather")

    if order.delivery_distance_km > 15:
        risks.append("Long delivery distance")

    if order.delivery_partner_experience_years < 2:
        risks.append("Inexperienced delivery partner")

    if order.refund_requested:
        risks.append("Refund requested")

    if order.tip_amount < 10:
        risks.append("Low tip amount")

    if order.estimated_delivery_time > 60:
        risks.append("Long estimated delivery time")
